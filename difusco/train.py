"""The handler for training and evaluation with constraint-aware decoding."""
...
def arg_parser():
    parser = ArgumentParser(description="Train a Pytorch-Lightning diffusion model on a TSP/MIS dataset.")
    ...
    parser.add_argument("--use_constraints", action="store_true",
        help="Enable constraint-aware decoding (e.g., feasibility checks, repair heuristics).")
...
def main(args):
    ...
    if args.use_constraints:
        constraint_manager = ConstraintManager(task=args.task)
        if hasattr(model, "decode") and callable(getattr(model, "decode")):
            original_decode = model.decode

            def constrained_decode(*decode_args, **decode_kwargs):
                raw_solution = original_decode(*decode_args, **decode_kwargs)
                return constraint_manager.apply(raw_solution)

            model.decode = constrained_decode
            rank_zero_info("Constraint-aware decoding enabled.")
    ...
    save_dir = wandb_logger.save_dir or "wandb_logs"
    checkpoint_callback = ModelCheckpoint(
        monitor="val/solved_cost", mode=saving_mode,
        save_top_k=3, save_last=True,
        dirpath=os.path.join(save_dir, wandb_logger.name, wandb_logger.version),
    )
    ...
    if args.do_train:
        if args.resume_weight_only and ckpt_path:
